...
@require_login(role=['admin'])
def api_agendamentos_por_profissional():
    """Retorna agendamentos por profissional dos últimos 30 dias"""
    try:
        from datetime import datetime, timedelta
        hoje = datetime.now()
        inicio = hoje - timedelta(days=30)
        ...
        return jsonify({
            ...
            'datasets': [{
                ...
                'label': 'Realizados',
                'data': [d.realizados for d in dados],
                'backgroundColor': 'rgba(25, 135, 84, 0.8)'
            }, {
                'label': 'Faltas',
                'data': [d.faltas for d in dados],
...
def api_receita_por_mes():
    ...
    try:
        ...
        query = db.session.query(
            ...
            db.func.strftime('%Y-%m', Agendamento.data)
        ).order_by('mes')
        ...
        for d in dados:
            if d.mes:
                ...
                receitas.append(float(d.receita or 0))
...
@require_login(role=['admin'])
def api_horarios_populares():
    """Retorna horários mais populares para agendamentos"""
    try:
        from datetime import datetime, timedelta
        hoje = datetime.now()
        ...
        return jsonify({
            'success': True,
            'labels': [d.hora for d in dados if d.hora],
            'datasets': [{
                'label': 'Agendamentos',
                'data': [d.total for d in dados],
                'backgroundColor': 'rgba(13, 110, 253, 0.8)'
...
@require_login(role=['admin'])
def api_status_agendamentos():
    """Retorna distribuição de status dos agendamentos"""
    try:
        from datetime import datetime, timedelta
        hoje = datetime.now()
        inicio = hoje - timedelta(days=30)
        ...
        return jsonify({
            'success': True,
            'labels': [d.status for d in dados if d.status],
            'datasets': [{
                'data': [d.total for d in dados],
                'backgroundColor': [cores.get(d.status or "", '#6c757d') for d in dados]
            }]
